          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 relative overflow-hidden">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <p className="text-lg font-medium text-gray-900">{client.nif || "--"}</p>
              </div>
               ...
               <table className="w-full text-sm text-left">
                 ...
                 <tbody className="divide-y">
                   {client.appointments.map(app => (
                     <tr key={app.id}>
                       <td className="p-3">{app.date.toLocaleDateString('pt-PT')}</td>
                       <td className="p-3">
                       ...
                       <td className="p-3 text-right font-bold">{Number(app.price).toFixed(2)}€</td>
                     </tr>
                   ))}
          ...
          <div className="bg-blue-50 p-6 rounded-xl border border-blue-100 sticky top-6">
            ...
            <form action={createPet} className="space-y-4">
              ...
              <div>
                ...
                <input name="name" required className="w-full p-2 rounded border border-blue-200 focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Ex: Bobby" />
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-bold text-blue-700 mb-1">Espécie</label>
                  <select name="species" className="w-full p-2 rounded border border-blue-200 bg-white">
                ...
                <div>
                  ...
                  <select name="gender" className="w-full p-2 rounded border border-blue-200 bg-white">
              ...
              <div>
                ...
                <input name="breed" className="w-full p-2 rounded border border-blue-200 focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Ex: Labrador" />
              </div>

              <div>
                <label className="block text-xs font-bold text-blue-700 mb-1">Data Nascimento (Aprox.)</label>
                <input name="birthDate" type="date" className="w-full p-2 rounded border border-blue-200 focus:ring-2 focus:ring-blue-500 outline-none" />
              </div>

              <div>
                <label className="block text-xs font-bold text-blue-700 mb-1">Microchip (Opcional)</label>
                <input name="microchip" className="w-full p-2 rounded border border-blue-200 focus:ring-2 focus:ring-blue-500 outline-none" />
              </div>

              <div>
                <label className="block text-xs font-bold text-blue-700 mb-1">Notas Médicas / Alertas</label>
                <textarea name="medicalNotes" rows={2} className="w-full p-2 rounded border border-blue-200 focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Ex: Alergia a frango..." />
              </div>
